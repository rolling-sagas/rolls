...
export const useModalStore = create((set) => ({
	modals: [],
	openModal: (content, backdropClose = true) =>
		set((state) => ({
			modals: [...state.modals, { content, backdropClose }],
		})),
	closeModal: () =>
		set((state) => ({
...
export function Modal() {
	...
	const closeModal = useModalStore((state) => state.closeModal);
	const closeModalAt = useModalStore((state) => state.closeModalAt);
		...
		<>
			{modals.map((modal, index) => (
				<div
					...
					onClick={() => modal.backdropClose && closeModalAt(index)}
				>
					{modal.content}
				</div>
			))}
