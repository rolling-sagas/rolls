...
import AssistantContent, { StreamerContent } from "./assistant";
...
const UserContentLine = ({ line }) => {
  ...
  if (line.startsWith("HINT: ")) {
    ...
    return (
      <div className="rs-text-secondary line-clamp-3">{line.substring(6)}</div>
    );
  }
...
export const Content = memo(({ id, role, content, status }) => {
  switch (role) {
    ...
    case "assistant":
      return <StreamerContent id={id} content={content} status={status} />;
